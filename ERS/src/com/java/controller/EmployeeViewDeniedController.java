...
@WebServlet("/deniedform") 
public class EmployeeViewDeniedController extends HttpServlet{
	...
	protected void doGet (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(session.getAttribute("Employee")==null) {
			...
			RequestForm denied_form = new RequestForm();
			EmployeeRepository empRp = new EmployeeRepository();
			Gson gson = new Gson();
			
			List<RequestForm> list_form = new ArrayList<>();//list of pending forms
			try {
				list_form = empRp.getDenied(emp.getEmployee_id()); //I want to use the object from that I store in session to return me the current employee id
