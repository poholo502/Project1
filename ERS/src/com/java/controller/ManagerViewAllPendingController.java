...
public class ManagerViewAllPendingController extends HttpServlet{
	...
	protected void doGet (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(session.getAttribute("Employee")==null) {
			...
			try {
				list_form = mgRp.getAllPending();
				response.setContentType("application/json");
				response.getWriter().println(gson.toJson(list_form));
				
			} catch (DatabaseException e) {
				e.printStackTrace();
				request.getRequestDispatcher("allpending").forward(request, response);
			}
	}
}
